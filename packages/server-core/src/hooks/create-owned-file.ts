...
export default () => {
  return async (context: HookContext): Promise<HookContext> => {
    ...
    if (body.projectId && context.params.previousFileId) {
      ...
      savedFile = reqArgs?.body?.skipStaticResource ? { id: '1234', mimeType: 'nothing', url: 'https://blank.com' } : await context.app.service('static-resource').create(modifiedResourceData)
    }
    context.result = {
