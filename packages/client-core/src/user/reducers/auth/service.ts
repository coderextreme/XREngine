...
export function uploadAvatarModel(model: any, thumbnail: any, avatarName?: string, isPublicAvatar?: boolean) {
  return async (dispatch: Dispatch, getState: any) => {
    ...
    const modelOperation = modelURL.local === true ? axios.post(`${Config.publicRuntimeConfig.apiServer}/media`, modelData, {
      headers: {
        'Content-Type': 'multipart/form-data',
        Authorization: 'Bearer ' + token
      }
    ...
    return modelOperation
      .then(async (res) => {
        console.log('model post response', res)
        ...
        const thumbnailOperation = thumbnailURL.local === true ? axios.post(`${Config.publicRuntimeConfig.apiServer}/media`, thumbnailData, {
          headers: {
            'Content-Type': 'multipart/form-data',
            Authorization: 'Bearer ' + token
          }
